...
async def check_turn(request: Request):
    ...
    try:
        ...
        prompt = """أنت نظام ذكاء اصطناعي فائق الدقة والصرامة متخصص حصرياً في تفريغ واستخراج لوحات السيارات السعودية بدقة 100%.
المطلوب بدقة متناهية وبدون أي تخمين:
1. ⚠️ ممنوع منعاً باتاً اختراع أو تخمين أي لوحة لم ينطقها المتحدث بوضوح تام. إذا كان هناك تشويش أو تنفس أو سكوت أرجع مصفوفة فارغة: []
2. استمع للصوت واكتب فقط اللوحة التي نطقها المتحدث حرفياً.
3. ⚠️ تمييز الحروف بدقة صوتية مطلقة (17 حرفاً فقط):
   - (ب أ ل) = باء ألف لام (مثال: 'ب أ ل 8502' ، لا تكتب أ ل ل أبداً).
   - (ح ن ق) = حاء نون قاف (مثال: 'ح ن ق 2385').
   - (ب ب د) = باء باء دال (مثال: 'ب ب د 2651').
   - (ح ن ط) = حاء نون طاء (مثال: 'ح ن ط 2419').
   - (ح م ك) = حاء ميم كاف (مثال: 'ح م ك 5467').
   - (ح و ط) = حاء واو طاء (مثال: 'ح و ط 7251').
   - (ح هـ ب) = حاء هاء باء (مثال: 'ح هـ ب 3575').
   - (ب أ ط) = باء ألف طاء (مثال: 'ب أ ط 6153').
4. ⚠️ الأرقام: اكتب جميع الأرقام كاملة (مثال: 'ثلاثة آلاف وخمسمائة وخمسة وسبعين' = 3575 ، 'ثمانية آلاف وخمسمائة واثنين' = 8502 ، 'ستة آلاف ومائة وثلاثة وخمسين' = 6153).
5. ⚠️ الصيغة القياسية: 3 حروف متباعدة + 1 إلى 4 أرقام (مثال: 'ب أ ط 6153').
6. أرجع مصفوفة JSON فقط بالشكل التالي:
[{"plate": "ب أ ط 6153", "found": false, "vehicle_type": "", "notes": ""}]
إذا كان الصوت صامتاً أو غير واضح أو تشويش، أرجع: []"""
