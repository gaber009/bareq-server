...
SPATIAL_LETTER_WORDS = [
    ...
    # ── Official Plate Letters & Phonetic Names ──
    ('جيم', 'ج'), ('جا', 'ج'), ('جم', 'ج'),
    ('حاء', 'ح'), ('حا', 'ح'),
    ('خاء', 'خ'), ('خا', 'خ'),
    ('تاء', 'ب'), ('تا', 'ب'),
...
SAUDI_VALID_LETTERS = set("أابجحخدرسصطعقكلمنههـويى")

# ═════════════════════════════════════════════════════════════════════
# Authentic Saudi License Plate Parser & Normalizer
# ═════════════════════════════════════════════════════════════════════
SAUDI_17_LETTERS = set("أابجحخدرسصطعقكلمنههـويى")
...
async def check_turn(request: Request):
    ...
    try:
        ...
        prompt = """أنت نظام ذكاء اصطناعي فائق الدقة والصرامة متخصص حصرياً في تفريغ واستخراج لوحات السيارات بدقة 100%.
المطلوب منك بدقة متناهية:
1. ⚠️ استمع للصوت واكتب بدقة تامة كل لوحة نطقها المتحدث بدون أي تفويت أو نقصان.
2. ⚠️ تمييز الحروف بدقة صوتية مطلقة:
   - احرص على عدم إسقاط الحرف الأول أو الأخير:
     * 'ألف صاد صاد' تُكتب (أ ص ص) وممنوع إسقاط الألف لتصبح (ص ص)!
     * 'سين حاء سين' تُكتب (س ح س) وممنوع إسقاط السين لتصبح (ح س)!
     * 'باء حاء طاء' تُكتب (ب ح ط) وممنوع إسقاط الطاء لتصبح (ب ح)!
   - التمييز الصوتي الدقيق بين الحروف:
     * 'باء ألف سين' تُكتب (ب أ س) ولا تُكتب (ب ل س)!
     * 'دال دال حاء' تُكتب (د د ح) وتكرار الحرف يكتب مكرراً ولا يُستبدل بلام!
     * إذا قال 'جيم' أو 'ج' اكتب (ج) ولا تحوله إلى حاء أبداً! (مثال: 'أ ب ج 1234').
     * إذا قال 'حاء' أو 'ح' اكتب (ح).
     * إذا قال 'كاف' اكتب (ك)، وإذا قال 'قاف' اكتب (ق).
     * إذا قال 'نون' اكتب (ن)، وإذا قال 'واو' اكتب (و).
3. ⚠️ دقة الأرقام: اكتب جميع الأرقام كما نُطقت بدقة شديدة (مثال: '9404' ، '7243' ، '6821' ، '9384' ، '4081' ، '60' ، '0001').
4. ⚠️ الصيغة القياسية: 3 حروف (أو حرفين) متباعدة + 1 إلى 4 أرقام (مثال: 'ب ح ط 6821' أو 'ب د ك 4081').
5. أرجع مصفوفة JSON فقط بالشكل التالي:
[{"plate": "ب ح ط 6821", "found": false, "vehicle_type": "", "notes": ""}]
إذا كان الصوت صامتاً تماماً، أرجع: []"""
        ...
        # Direct fast invocation of Gemini using active primary model
        target_model = cfg.get("gemini_model", "gemini-3.6-flash")
        res_json = await asyncio.to_thread(_call_gemini_with_rotation, cfg, payload, target_model, "live")
        raw_text = res_json["candidates"][0]["content"]["parts"][0]["text"].strip()
        if raw_text.startswith("```json"): raw_text = raw_text[7:]
        if raw_text.startswith("```"): raw_text = raw_text[3:]
        if raw_text.endswith("```"): raw_text = raw_text[:-3]
