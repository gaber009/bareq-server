...
async def check_turn(request: Request):
    ...
    try:
        ...
        prompt = """أنت نظام ذكاء اصطناعي فائق الدقة متخصص حصرياً في تفريغ واستخراج لوحات السيارات السعودية بدقة 100% وبدون أي تخمين.
المطلوب بدقة متناهية:
1. استمع للصوت واكتب اللوحة التي نطقها المتحدث كما نُطقت تماماً.
2. ⚠️ تمييز الحروف الصوتية بدقة متناهية (17 حرفاً معتمداً فقط):
   - (صاد = ص) مقابل (سين = س): ميز بدقة صفير الحرف (مثال: 'ح س ص' = حاء سين صاد ، 'ح ص ص' = حاء صاد صاد ، 'ح س ع' = حاء سين عين).
   - (قاف = ق) مقابل (طاء = ط): ميز بدقة (مثال: 'ح ن ق' = حاء نون قاف ، 'ح ن ط' = حاء نون طاء).
   - (حاء = ح) مقابل (هاء = هـ / واو = و): (مثال: 'ح و و' = حاء واو واو ، 'ح ن أ' = حاء نون ألف).
   - (ميم = م) مقابل (نون = ن): (مثال: 'ح ق م' = حاء قاف ميم ، 'ح ق ن' = حاء قاف نون).
   - (دال = د) مقابل (لام = ل / باء = ب): (مثال: 'ب د ل' = باء دال لام ، 'د ح ب' = دال حاء باء).
   - (راء = ر): (مثال: 'ر ح م' = راء حاء ميم).
3. ⚠️ الأرقام: اكتب جميع الأرقام كما نُطقت تماماً بدون إسقاط أي رقم (مثال: 6626 ، 4318 ، 5835 ، 6324 ، 9913).
4. ⚠️ الصيغة القياسية: 3 حروف متباعدة + 1 إلى 4 أرقام (مثال: 'ر ح م 3830' أو 'ح س ص 5118' أو 'ب د ل 4641').
